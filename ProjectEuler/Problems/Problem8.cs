...
namespace ProjectEuler.Problems
{
	...
	{
		private string value = @"73167176531330624919225119674426574742355349194934
		...
		public override void Go()
		{
			...
			for (var i = 0; i < value.Length - searchRegion; i++)
			{
				var result = new Result
				{
					SearchValue = value.Substring(i, searchRegion)
				};

				if (result.SearchValue.Contains("0"))
				{
					i += result.SearchValue.IndexOf('0');
				}
				else
				{
					result.MultiplicationResult = 1;
					foreach (var numeral in result.SearchValue)
					{
						result.MultiplicationResult *= Convert.ToInt16(numeral.ToString());
					}
				}
